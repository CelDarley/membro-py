...
def include_object(object, name, type_, reflected, compare_to):
    # Apenas permitir alterações em tabelas gerenciadas
    if type_ == 'table':
        return name in MANAGED_TABLES
    if type_ in ('index', 'column', 'constraint'):
        table_name = getattr(object.table, 'name', None) if hasattr(object, 'table') else None
        if table_name:
